...
const AppLogo: React.FC = () => (
  <div className="relative group">
    ...
    <img 
      src={logoSrc} 
      alt="Bharat Path Logo" 
      className="w-32 h-32 rounded-[2.5rem] shadow-[0_0_50px_rgba(249,115,22,0.3)] relative z-10 transition-transform duration-1000 group-hover:scale-110" 
    />
  </div>
);
...
const MandalaSVG: React.FC<{
  ...
  reverse?: boolean;
  opacity?: number;
}> = ({ className, rotationSpeed = '60s', scale = 1, reverse = false, opacity = 1 }) => (
  <svg 
    viewBox="0 0 200 200" 
...
const WelcomePage: React.FC<WelcomePageProps> = ({ onComplete }) => {
  ...
  // FIX: Redirect after animation with safety check
  useEffect(() => {
    const timer = setTimeout(() => {
      if (onComplete) onComplete();
    }, 4000); // Thoda buffer rakha hai 4s ka smooth transition ke liye
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-[#020208] p-4 overflow-hidden relative selection:bg-orange-500/30 font-sans">
      ...
      <style>{`
        /* FIX: Added scale into keyframes to prevent Rollup build errors */
        @keyframes rotate { 
